...
const mapping = { h2: 'section-title', h3: 'question', p: 'answer', ul: 'answer' };
...
function html() {
    ...
    config.sections.forEach((section) => {
        ...
        section.parts.forEach((part) => {
            markdownSource += "\n## " + part.name + "\n\n";
            topMenuHtmlSource += '<a class="nav-link scrollto" href="#' + markdownItAnchor.defaults.slugify(part.name) + '">' + part.name + '</a>';
            part.questions.forEach((question) => {
                markdownSource += '<div class="section-block">\n\n';
