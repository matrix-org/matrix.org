...
lexicographically smaller device ID is used. This ensures that unambiguous which `m.key.verification.start`
is used. From now on, the device whos `m.key.verification.start` is used is the one who started the
...
{
  "method": "m.sas.v1",
  "key_agreement_protocol": "curve25519-hkdf-sha256", // if the other party doesn't support this, pick "curve25519" instead
  "hash": "sha256",
...
cache and which not, based on what kind of security it can offer. For cross-signing that needed secrets
