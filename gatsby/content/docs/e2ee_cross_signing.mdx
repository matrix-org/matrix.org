...
While the to_device messages are sent encrypted, the room messages don't *have* to be encrypted - it
depends on if the room they are sent in is encrypted or not.
...
{
  "method": "m.sas.v1",
  "key_agreement_protocols": ["curve25519-hkdf-sha256", "curve25519"], // while "curve25519" is deprecated, it is still wildly used and thus needs to be supported
  // Please note that "curve25519" should not be implemented - this guide in pull-request form still covers it
  // in case someone is using the pull request guide to implement stuff.
  // You should probably remove "curve25519" completely after approximately September 2020, as then a reasonable
  // long time passed for other clients to update.
  "hashes": ["sha256"],
...
##### `curve25519`
IMPORTANT! This section will be removed before the guide is published. If it is after September 2020,
...
is the lack of public keys and the lack of a `|` delimiter. In code that could look as follows:
...
Important: Some PBKDF2 implementations take as length the length in bytes (like here), while others take
