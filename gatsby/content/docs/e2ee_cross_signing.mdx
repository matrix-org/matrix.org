...
for said secret, that requires user interaction. As such, if a client can store a secret seasonably
securely client-sided, it is recommended to cache some secrets locally. For cross-signing that would
be `m.cross_signing.self_signing`, `m.cross_signing.user_signing` and for online key backup that would
be `m.megolm_backup.v1`. Blindly caching all secrets is a security risk, as some secrets such as
`m.cross_signing.master` are highly sensitive and shouldn't be stored in plaintext on any disk, ever.
...
to_device event. Please keep in mind that these all should be encrypted! To do your part in the sharing
...
3. When receiving a secret, verify that it was from one of the devices you sent the secret to (possible
...
keys. This is done either by password or by keyfile, afterwards the `m.secret_storage.key.<keyId>` is
...
these parameters with the hash algorithm sha512 to generate a key:

```dart
Uint8List keyFromPassword(String password, _PasswordInfo info) {
...
Important: Some PBKDF2 algorithms take as length the length in bytes (like here), while others take
...
Note: Encrypting and decrypting both uses *unpadded* AES-CTR. If your library does not have an option
for unpadded AES-CTR en/decryption, simply append prior en/decryption 0's to your data to fill up the
input to a full number of blocks. One block here is typically 32? bytes in size. After en/decrypting,
strip the extra bytes again. Please note that this method will *not* work with all ciphers (e.g. XTS), it is
something specific to CTR.
...
It is a series of specific sha256 hmac hashes, as following:

```dart
_DerivedKeys deriveKeys(Uint8List key, String name) {
  var zerosalt = Uint8List(8); // generate 8 bytes of zero
...
String decryptAes(_Encrypted data, Uint8List key, String name) {
  ...
  // decrypt the cipher with the generated aesKey, using unpadded AES-CTR and the supplied IV
