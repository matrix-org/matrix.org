...
This removes all local users from the room, routing them into a new room (called ‘Room unavailable’ in this example), repoints any local aliases to the new room, blacklists the server from participating in the original room and prevents any local user from ever rejoining the room.
