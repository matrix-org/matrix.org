...
while len(incoming_edges) > 0:
    for each sorted_incoming_edges() as key -> value:
        if value == 0:
            output_event.prepend(event_map[key])

    for each event_map[key].auth_events as auth_event:
        incoming_edges[auth_event.event_id] -= 1

    remove(incoming_edges, key)
```
...
Now that our control events have been sorted into reverse topological order, where the first events in the list are the "oldest", we can now start to layer these resolved control events on top of the partial state.
