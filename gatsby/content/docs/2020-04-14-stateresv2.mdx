...
To build up the partial state, start at the oldest event and loop through the list, authing each event against the current `partial_state` (with this behaviour defined as `auth_against_partial_state` in the below example). It is expected that `partial_state` is an empty list for the first iteration, as the oldest event (which is the room create event) does not have auth events. The partial state will grow with each iteration.
