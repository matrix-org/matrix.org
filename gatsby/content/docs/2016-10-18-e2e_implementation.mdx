...
currently enforce that in Synapse).
...
type `m.room.encryption`\_, and content
...
:   The user ID of the sender. The client should check that this matches
    the `sender` in the event.

`recipient`

:   The user ID of the recipient. The client should check that this
    matches the local user ID.

`keys`

:   an object with a property `ed25519`. The client should check that
    the value of this property matches the sender\'s fingerprint key
    when [marking the event as
    verified](#marking the event as verified).
...
[below](#m.room_key)), the ciphertext can be decrypted by passing the
...
### Handling an `m.room_key` event {#m.room_key}
...
The `room_id`, together with the `sender_key` of the `m.room_key`\_
...
of the encrypted `m.room_key`\_ event and store it with the inbound
session. This is used as above when marking the event as verified.

### Downloading the device list for users in the room {#download the device list}
...
required to allow users to [verify or block devices](#blocking).
...
an m.room\_key event](#m.room_key).

The client must then share the keys for this session with each device in
the room. It must therefore [download the device
list](#download the device list) if it hasn\'t already done so. Then it
should build a unique `m.room_key`\_ event, and send it encrypted [using
Olm](#olm_encrypt) to each device in the room which has not been
[blocked](#blocking), .
...
`m.room.encryption`\_ state event of a room.
...
### Encrypting an event with Olm {#olm_encrypt}
...
-   Build an encryption payload as illustrated in the [spec](#blocking).
...
-   Check if it has an existing Olm session; if it does not, [start a
    ...
    happen due to races when establishing sessions), it should use the
    one with the first session\_id when sorted by their ASCII codepoints
    (ie, \'A\' would be before \'Z\', which would be before \'a\').

    \_\_ [Starting an Olm session](#starting-an-olm-session)
...
device list](#download the device list) for the new member, if it
...
### Blocking / Verifying devices {#blocking}
...
session]{.title-ref}\_\_. Any active outbound Megolm sessions whose keys
have been shared with the device should also be invalidated so that no
further messages are sent over them.

### Marking events as \'verified\' {#marking the event as verified}
...
The `session_key` property of a `m.forwarded_room_key`\_ event differs
...
`m.room_key_request`\_ event to all the devices it requested the key
