---
layout: post
title: matrix-js-bot-sdk Room Admin Features
section: guides
---

## Intro

[matrix-js-bot-sdk] has features which enable bots to perform administration of Matrix roooms - tasks like kicking, banning and inviting users. Of course, the bot must have the appropriate permissions for the room in order for this to work.

You should read [Usage of matrix-js-bot-sdk] for a more fundamental introduction to the SDK.

## Kicking

Kicking a user means that user is removed from the room by force, but is not banned - so they are able to re-join.

Once you have your client object running (see [usage-of-matrix-js-bot-sdk#instantiation](https://matrix.org/docs/guides/usage-of-matrix-js-bot-sdk#instantiation)), you can use the `kickUser()` method. There are three parameters:
...
In [usage-of-matrix-js-bot-sdk#implementing-echobot-functionality](https://matrix.org/docs/guides/usage-of-matrix-js-bot-sdk#implementing-echobot-functionality), we showed how it's possible to read the content of messages and respond to them. To reiterate:
...
[matrix-js-bot-sdk] is promise-based, so you can chain `.then()` and `.catch()` methods to your Matrix calls to respond after they return:
...
[matrix-js-bot-sdk] can do a great deal when it comes to room administration. Check out [the repo][matrix-js-bot-sdk] directly to find out more - or if you need a more introductory guide see [Usage of matrix-js-bot-sdk].

[matrix-js-bot-sdk]: https://github.com/turt2live/matrix-js-bot-sdk
[Usage of matrix-js-bot-sdk]: https://matrix.org/docs/guides/usage-of-matrix-js-bot-sdk
