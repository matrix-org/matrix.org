...
If you're using Riot, clicking on a user to bring up more information about them will show a 'remove recent messages' button which allows you to redact the user's events. It'll only get the events that Riot can see, but can help when a room is being spammed by a bad actor.
