...
categories: 
 - client
description: A free email, calendar & chat app
author: Mozilla
maturity: Beta
language: C
license: MPL-2.0
...
Matrix support in Thunderbird is enabled by default in Nightly builds (as of Thunderbird 91), but can be enabled in Beta and ESR builds by setting `chat.prpls.prpl-matrix.disable` to `false` in the advanced config editor (>Tools >Options >Advanced >Config Editor).
