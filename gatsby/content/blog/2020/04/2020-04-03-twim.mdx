...
Google closed the door to applications on Tuesday, and we have an absolute bounty of proposals. More information will be shared when we announcements are made in a few weeks.
...
> * We now have an `Error` type that error responses from the homserver will be deserialized into
