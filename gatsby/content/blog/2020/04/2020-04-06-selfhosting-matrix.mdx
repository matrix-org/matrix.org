 ...
 * Go to `/etc/nginx/sites-enabled` and copy the vhost configuration block from the bottom of `default` to new files called `dangerousdemos.net`, `matrix.dangerousdemos.net`, and `riot.dangerousdemos.net`.
   * Rename the `server_name` field in the new files to match the hostname of each host, and point `root` to an appropriate location per domain (e.g. `/var/www/dangerousdemos.net`)
...
# e.g. https://keyserver.ubuntu.com/ and search for releases@riot.im.
# This should give a keyID like 74692659bda3d940, which you can then import:
gpg --keyserver keyserver.ubuntu.com --recv-keys 74692659bda3d940

# alternatively, you can grab or cross-check the signing key from packages.riot.im
 ...
 * We give the installer the hostname `jitsi.dangerousdemos.net`.
