...
improving Synapse's performance) - so it's more important than ever to help
folks run their own servers.
...
Finally, while the video shows how to install on Debian via Debian packaages,
...
If you want to follow along at home without listening to the video (and I can't blame you ;), the high level steps are as follows:
 ...
 * Go to `/etc/nginx/sites-enabled` and copy the vhost configuration block from the bottom of `default` to new files called `dangerousdemos.net`, `matrix.dangerousdemos.net`, and `riot.dangerousdemos.net`.
   * Rename the `server_name` field in the new files match the hostname of each host, and point `root` to an appropriate location per domain (e.g. `/var/www/dangerousdemos.net`)
 ...
 * Then, install Synapse via Debian packages using the instructions at https://github.com/matrix-org/synapse/blob/master/INSTALL.md#debianubuntu (see below)
...
wget https://packages.riot.im/riot-release-key.asc # ideally we'd get this from a keyserver
apt install -y gpg
 ...
 * We give the install the hostname `jitsi.dangerousdemos.net`.
 * The installer magically detects you have nginx installed and adds in an appropriate vhost!
 * We select a self-signed certificate for now, and then upgrade it to LetsEncrypt after the event with `/usr/share/jitsi-meet/scripts/install-letsencrypt-cert.sh`.
...
Matrix provides an excellent alternative to the centralised solutions these days.  It gives:
...
And if that's still too scary, please take a look at one of the hosting services.
