...
>
> @TravisR wrote a [fix for Riot Web](https://github.com/matrix-org/matrix-js-sdk/pull/1304), so you should be able to create accounts on Conduit from [https://riot.im/app](https://riot.im/app) soon.
>
...
> I made a TARDIS (Time Agnostic Room DAG Inspection Service) - a simple time-travelling debugger for Room DAGs which plugs into Dendrite's internal APIs to visualise room DAGs.  The intention is to provide it as a widget which Riot can use to visualise DAGs for debugging, particularly for P2P Matrix.  The time-travel bit is simply that it could let you filter out the newer events to show how the DAG has evolved over time, although that isn't hooked up yet. [https://github.com/matrix-org/tardis](https://github.com/matrix-org/tardis)
...
> I, Half-Shot, changed the status of the IRC Bridge on [https://matrix.org/docs/projects/bridge/matrix-org-irc-bridge](https://matrix.org/docs/projects/bridge/matrix-org-irc-bridge) from "Early beta" to "Stable". This does not mean it's feature complete, or even all the way there, but it sure doesn't crash nearly as often as it did when I started, so that's good enough for me.
...
>
> Try it at [https://daydream.nordgedanken.dev/](https://daydream.nordgedanken.dev/) or check out the Source Code at [https://github.com/MTRNord/Daydream](https://github.com/MTRNord/Daydream)
>
...
>
>
> See [https://github.com/midoricorp/jabbot/blob/master/README.md](https://github.com/midoricorp/jabbot/blob/master/README.md) for details
>
...
> After explaining cross signing to others multiple times in the past and seeing the question again on mastodon, I decided to write a short blog post on the topic, explaining how the keys interact with each other and how Alice's devices then end up trusting Bob's devices. You can read it over on [https://jcg.re/blog/quick-overview-matrix-cross-signing/](https://jcg.re/blog/quick-overview-matrix-cross-signing/)
