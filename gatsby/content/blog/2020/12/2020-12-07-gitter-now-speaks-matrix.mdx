...
It’s been just over 2 months since we revealed that [Gitter was going to join Matrix](https://matrix.org/blog/2020/09/30/welcoming-gitter-to-matrix) - and we are incredibly proud to announce that** **Gitter has officially turned on true native Matrix connectivity: **all public Gitter rooms are now available natively via Matrix, and all Gitter users now natively exist on Matrix.**  So, if you wanted to join the official Scala language support room at [https://gitter.im/scala/scala](https://gitter.im/scala/scala) from Matrix, just head over to [#scala_scala:gitter.im](https://matrix.to/#/#scala_scala:gitter.im) and_ *boom*, _you’re in!
...
5. Figuring out how to namespace the mxids of the gitter users:
    4. It’s nice to have the mxid as human readable as possible instead of just the numerical userId in your service.
    5. But if people can change their username in your service, you can’t change your mxid on Matrix.  In the future, we’ll have portable accounts in Matrix to support this ([MSC2787](https://github.com/matrix-org/matrix-doc/pull/2787)) but sadly these are still vapourware at this point.
    6. If you naively just switch the user’s mxid when they rename their username, then you could end up leaking conversation history between mxids(!)
    7. So we went with @username-userid:gitter.im for the Matrix ID to make it a bit more human readable but also unique so any renames can happen without affecting anything.
...
7. Figuring out how to bridge features correctly;
    8. Emoji - mapping between :shortcode: and unicode emojis
    9. Mapping between Gitter threaded conversations &lt;-> Matrix replies
    10. Mapping between Matrix mentions and Gitter mentions
8. Keeping users and room data in sync
    11. We haven’t gotten there yet, but the data comes through the same Mongoose hook and we can update the bridged data as they change on the Gitter end.
