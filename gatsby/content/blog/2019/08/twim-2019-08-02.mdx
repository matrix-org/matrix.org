...
(kudos [anoa] for providing this well-formatted summary section)
...
>This week we’ve been working on implementing identifier hashing in sydent, a brand new installer for Synapse, we’ve trialled some new db perf improvements which look very promising (more on this soon) and finally a very close to an all new implementation of room search.
>
>Next week we’ll push out the all new Sygnal, continue with the installer and room search and pick up some tasks under our pracy umbrella to ensure that Synapse is not holding onto any data that it does not strictly need to.
...
>
> Cnly’s GSoC period ends on August 26th. He has been a massive boon to the project’s cadance so far and we hope he will continue even after GSoC ends when he has time :)
