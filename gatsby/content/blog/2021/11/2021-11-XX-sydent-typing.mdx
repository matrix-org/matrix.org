# Type coverage for Sydent: lessons learnt
...
<figcaption>
The grey region above shows the two-week sprint towards improving coverage. The earliest data point is from just before our earlier efforts to improve coverage this summer.
</figcaption>
</figure>
...
(https://mypy.readthedocs.io/en/stable/)—a great way to spot bugs before you ship them.
...
async def bar():
    x = foo()      # no error
    y: int = foo() # error: rhs is Awiatable[int], but lhs expects int
...
Our function returns a `JsonDict`, which is an [alias](https://github.com/matrix-org/sydent/blob/2e3b7576b17e92080319e08c0c0ebf566935636c/sydent/types.py#L17) for `Dict[str, Any]`. This is bad news, because `sid: Any` holds true for both `sid: int` and `sid: Awaitable[int]`—so there's no error to spot here. The `Any` type is compatible with any other type whatsoever. Mypy uses it as a way to [defer all type checking to runtime](https://mypy.readthedocs.io/en/stable/kinds_of_types.html#the-any-type); mypy won't (and can't!) statically analyse the usage of an expression of type `Any`. Indeed, mypy's reports will tell you how many `Any`s you're working with, and offer a variety of options to [warn or error on usages of `Any`](https://mypy.readthedocs.io/en/stable/command_line.html#disallow-dynamic-typing).
...
This is a decent way to measure your progress when working on a particular module or package, but it's not perfect. The errors here aren't independent. Fixing one could fix another ten or reveal another twenty—the numeric value can be erratic.
...
This produces a main index page showing the "imprecision" of each module. At the bottom of the table is a total imprecision value across the entire project.
  ...
  ![HTML report, index page. A table showing each module's precision and number of lines of code.](/blog/img/2021-11-XX-sydent-typing-html-report-index.png)
...
The precision for each module is broken down line-by-line and colour-coded accordingly, which is useful for getting an intuition for what makes a line imprecise. More on that shortly.
  ...
  ![HTML report, module page. Most lines of source code are highlighted green; a minority are highlighted red.](/blog/img/2021-11-XX-sydent-typing-html-report-module.png)
...
After the sprint to improve coverage, I spent a short amount of time trying the alternative type checkers out there. This is by no means an authoritative, exhaustive survey—just my quick notes.
