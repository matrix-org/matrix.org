...
"Typing" refers to the process of annotating Python source code, so that automated tools can reason about the types of expressions that flow through a program. These annotations can be automatically checked by a tool like [`mypy`](https://mypy.readthedocs.io/en/stable/) — a great way to spot bugs before you ship them.
