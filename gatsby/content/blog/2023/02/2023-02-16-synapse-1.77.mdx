...
[MSC3952](https://github.com/matrix-org/matrix-spec-proposals/pull/3952) is a way to solve that trouble
by making the mentioning explicit in the protocol using a dedicated event property,
...
Have you already been annoyed by a noisy notification that comes back, but you can't really pinpoint
from where this is coming ?
This is usually because currently, editing a message where you are mentioned (or including an `@room`) will trigger
a noisy notification, which can be pretty annoying when the message is edited 10 times.
