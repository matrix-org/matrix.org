...
> it’s matrix summer special: https://matrix.org/blog/2022/08/15/the-matrix-summer-special-2022
...
> 
> Widgets are in a bit of a strange state - they're still not yet part of the spec proper, but are iterated upon in implementation. This leaves them in a similar spot to how aggregations were (and for a portion of, still are); in that they were in use in clients but hadn't yet landed in the spec.
> 
> Much like aggregations, it will be a lot of work to eventually land widgets in the spec. But also much like aggregations, I am optimistic for it eventually crossing the line!
> 
> With that detour out of the way, let's highlight this MSC! It solves a crucial problem with a simple solution. Widgets can ask the client they're embedded in to do certain things (if granted certain capabilities), and the client, potentially after asking the user for permission, can allow or deny those actions. This MSC adds the machinery for a further step: the client will tell the widget what capabilities they requested were allowed, and which were denied.
> 
