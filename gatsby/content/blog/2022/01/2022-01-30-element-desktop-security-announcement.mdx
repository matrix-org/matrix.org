...
However in certain platform configurations, the same vulnerability could allow an attacker to open an arbitrary URL with an arbitrary scheme, again using the platform's standard mechanisms. In some situations, the ability to open arbitrary URLs [has been shown](https://positive.security/blog/url-open-rce) to lead to arbitrary code execution.
...
Patched in 1.9.7 with further hardening done in 1.9.9 to ensure it's harder to exploit even in light of new Electron vulnerabilities. Please upgrade to 1.9.9 as soon as possible.
