---
date: '2022-02-03'
title: 'Digital Markets Act and interoperability: myth debunking of the gatekeepers arguments'
categories:
  - General
...
⇒ **you don’t have to**: You could leverage existing technologies which are being used by the industry. Matrix is the only one which could be called an open standard today, but there are others openly specified like XMPP for example. Matrix has been managed by its own standard body (The Matrix Foundation) and could be handed over to a more established one like IETF, ETSI or W3C if needed.
