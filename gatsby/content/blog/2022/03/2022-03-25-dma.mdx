...
garden again, or arbitrarily close down or sabotage their APIs.

### So, what’s the catch?

Since the DMA announcement on Thursday, there’s been [quite a lot]
(https://twitter.com/alexstamos/status/1507145126006587411) of [yelling]
(https://twitter.com/AlecMuffett/status/1507366934412734491) from some very
...
* We could run the bridge somewhere relatively safe - e.g. the user’s client.
  ...
  relays the messages into Matrix once decrypted.  By decentralising the
...
* The gatekeeper could switch to a decentralised end-to-end encrypted protocol
  ...
  it out.  (We’d ideally need to figure out decentralised/federated
  identity-lookup first though, to avoid switching from one centralised
  identity database to another).
...
and people can turn up.  And much like real life, this is a solvable problem,
...
(https://element.io/blog/moderation-needs-a-radical-change/), which outlines
one strategy - but there are many others.  Honestly, having to improve
...
the risks, and we should be celebrating a new dawn for open APIs rather than
fearing that the sky is falling and this is some evil attack on end-to-end
encryption!
