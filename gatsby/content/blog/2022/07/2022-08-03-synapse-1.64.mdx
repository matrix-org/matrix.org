...
external identity server. This validation is used by Synapse when associating
email addresses to a Matrix account, or before performing a password reset.

As of Synapse 1.64, this option is deprecated. This is because this option
relies on old API endpoints that have since been removed from the Matrix
specification. Server administrators who still need the ability to validate the
ownership of email addresses must configure an SMTP server in their Synapse
...
expected to be released in 4 weeks.
...
supported in this case, and a long-standing pain point for server administrators
has been the lack of support for implicit TLS (i.e. the use of TLS without a
STARTTLS upgrade).

Synapse 1.64 introduces support for implicit TLS. As of this version of Synapse,
server administrators can set the new configuration option `force_tls` to `true`
in the `email` section of their configuration file.

See the [configuration
guide](https://matrix-org.github.io/synapse/v1.64/usage/configuration/config_documentation.html#email)
for more information. Note that the `require_transport_security` setting of the
`email` section of the configuration file is ignored when implicit TLS is
enabled.

## Everything else

Synapse 1.64 features a new rate limiter to limit the rate of joins to the same
...
admin APIs to incude the type of a room in responses, allowing server
administrators to differenciate spaces from other rooms.

We also encourage server administrators to upgrade their local version of
[frozendict](https://pypi.org/project/frozendict/) to version 2.3.3 or higher,
as this version fixes a memory leak that occurs during `/sync` requests. The
