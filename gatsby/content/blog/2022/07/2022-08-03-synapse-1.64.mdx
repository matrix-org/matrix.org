...
relies on old API endpoints that have since been
[removed](https://spec.matrix.org/v1.1/changelog/#identity-service-api) from the
Matrix specification. Server administrators who still need the ability to
validate the ownership of email addresses must configure an SMTP server in their
Synapse configuration file. See the [configuration
