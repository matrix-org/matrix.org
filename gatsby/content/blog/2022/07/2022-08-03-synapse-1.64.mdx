...
As of Synapse 1.64, this option is deprecated, and Synapse will print a warning if it is used.
This is because this option
relies on old API endpoints that have since been removed from the Matrix
specification.

Synapse can do this validation internally provided it is configured with
details of an SMTP server. Administrators currently relying on
`account_threepid_delegates.email` should therefore ensure that an
SMTP server is correctly configured, and remove the
`account_threepid_delegates.email` option. See the [configuration
