...
One common scenario of abuse we have seen on Matrix over the years is attackers
...
entirely and release it as Matrix "spaces". If you're curious about this, then
you should probably read the [spaces announcement
...
groups from Synapse, after more than 4 years of good and loyal services. This
release of Synapse deprecates the feature and disables it by default, with a
plan to completely remove it in Synapse 1.58 (which should go out in a couple of
weeks).

## Everything else

As of this version, Synapse will also refuse to start the database it is running
against has a non-`C` locale. See the [upgrade
...
to allow modules to register existing 3PID associations.
