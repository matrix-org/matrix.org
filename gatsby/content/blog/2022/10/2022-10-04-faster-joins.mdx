...
---
We’re very excited to announce that, as of Synapse 1.69, "faster remote room joins" are ready for community testing.
...
![Element screenshot](/blog/img/2022-10-04-joining-hq.png)
 ...
 * Clients which don't support [lazy-loading room members](https://spec.matrix.org/v1.4/client-server-api/#lazy-loading-room-members) will *block* (ie, they won't receive any new events at all) while the resynchronisation process ([synapse#12989](https://github.com/matrix-org/synapse/issues/12989)).

## Turning it on

With all that said: we are very excited for people to start trying it and giving us feedback. If you administer a Synapse server and are as excited as we are, all you need to do to start testing faster joins is to add this to your `homeserver.yaml` (and then restart Synapse):
