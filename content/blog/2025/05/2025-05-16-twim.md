...
### Rory&::MatrixUtils (General user/moderation/admin toolkit for matrix)
...
### Rory&::MatrixAntiDmSpam (user tooling for invite spam)
> 
...
> * Moved around quite a few config options, as well as implementing proper support for slowing down the sync loop (to prevent causing more server load than nessecary), and overriding presence to offline by default.
