...
> 
> Along with Circles iOS, we are also announcing the beta phase for its SDK, [Matrix.swift](https://gitlab.futo.org/circles/matrix.swift).  The latest version v0.4.0 includes support for message redaction and replacment, detection of `@` mentions in message events, and support for encrypting and uploading video files for `m.video` messages.  The Matrix.swift SDK is now at the point where it could be a useful building block for hobby projects and open source apps.
> 
