...
Thankfully, supporting authenticated media should be relatively trivial for most clients. When the homeserver declares support for Matrix 1.11 through <code>[/versions](https://spec.matrix.org/v1.11/client-server-api/#get_matrixclientversions)</code>, the client should use the new <code>[/_matrix/client/v1/media/* endpoints](https://spec.matrix.org/v1.11/client-server-api/#content-repo-client-behaviour)</code>, noting that they now require authentication. The user’s access token can be supplied using the <code>Authorization</code> header. (Note that using the query string <code>?access_token</code> method is not recommended, and [deprecated](https://spec.matrix.org/v1.11/client-server-api/#using-access-tokens) in Matrix 1.11 for all endpoints.)
